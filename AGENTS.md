...
entry point for **Codex** and any agent that reads `AGENTS.md`; Claude Code reads
[CLAUDE.md](CLAUDE.md), which says the same thing. Both point at one contract:
...
```bash
./scripts/setup-agent.sh <your-name>      # e.g. ./scripts/setup-agent.sh codex
```
